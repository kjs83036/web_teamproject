...
public interface MyPageDao {
    ...
    @Update("UPDATE STORES SET STORENUMBER=#{storeNumber}, STORENAME=#{storeName}, ADDRESS=#{address}, PHONENUMBER=#{phoneNumber}, IMAGES=#{images} WHERE SEQ=#{seq}")
    public int updateStore(Store store);
    
    @Update("UPDATE STORES SET STORENUMBER=#{storeNumber}, STORENAME=#{storeName}, ADDRESS=#{address}, PHONENUMBER=#{phoneNumber} WHERE SEQ=#{seq}")
    ...
    //가게 주문 내역 뽑기
    @Select(" SELECT ORDERDATE,STORENAME,MENUNAME,AMOUNT,O.PRICE AS PRICE,O.TIME AS PICKUPTIME,ORDERSEQ,STATEMENT,O.REQUIREMENTS AS REQUIREMENTS FROM ORDERS O JOIN MENUS M ON O.MENUSEQ=M.MSEQ JOIN STORES S ON S.SEQ=M.SSEQ WHERE S.SEQ=#{seq} ORDER BY ORDERDATE DESC")
